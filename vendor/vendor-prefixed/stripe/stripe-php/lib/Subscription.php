 ...
 * @property null|string|\StellarPay\Vendors\Stripe\Application $application ID of the Connect Application that created the subscription.
 ...
 * @property null|\StellarPay\Vendors\Stripe\StripeObject $billing_cycle_anchor_config The fixed values used to calculate the <code>billing_cycle_anchor</code>.
 * @property null|\StellarPay\Vendors\Stripe\StripeObject $billing_thresholds Define thresholds at which an invoice will be sent, and the subscription advanced to a new billing period
 ...
 * @property null|\StellarPay\Vendors\Stripe\StripeObject $cancellation_details Details about why this subscription was cancelled
 ...
 * @property string|\StellarPay\Vendors\Stripe\Customer $customer ID of the customer who owns the subscription.
 ...
 * @property null|string|\StellarPay\Vendors\Stripe\Account|\StellarPay\Vendors\Stripe\BankAccount|\StellarPay\Vendors\Stripe\Card|\StellarPay\Vendors\Stripe\Source $default_source ID of the default payment source for the subscription. It must belong to the customer associated with the subscription and be in a chargeable state. If <code>default_payment_method</code> is also set, <code>default_payment_method</code> will take precedence. If neither are set, invoices will use the customer's <a href="https://stripe.com/docs/api/customers/object#customer_object-invoice_settings-default_payment_method">invoice_settings.default_payment_method</a> or <a href="https://stripe.com/docs/api/customers/object#customer_object-default_source">default_source</a>.
 * @property null|\StellarPay\Vendors\Stripe\TaxRate[] $default_tax_rates The tax rates that will apply to any subscription item that does not have <code>tax_rates</code> set. Invoices created will have their <code>default_tax_rates</code> populated from the subscription.
 ...
 * @property null|\StellarPay\Vendors\Stripe\Discount $discount Describes the current discount applied to this subscription, if there is one. When billing, a discount applied to a subscription overrides a discount applied on a customer-wide basis. This field has been deprecated and will be removed in a future API version. Use <code>discounts</code> instead.
 * @property (string|\StellarPay\Vendors\Stripe\Discount)[] $discounts The discounts applied to the subscription. Subscription item discounts are applied before subscription discounts. Use <code>expand[]=discounts</code> to expand each discount.
 ...
 * @property \StellarPay\Vendors\Stripe\Collection<\Stripe\SubscriptionItem> $items List of subscription items, each with an attached price.
 * @property null|string|\StellarPay\Vendors\Stripe\Invoice $latest_invoice The most recent invoice this subscription has generated.
 ...
 * @property null|string|\StellarPay\Vendors\Stripe\Account $on_behalf_of The account (if any) the charge was made on behalf of for charges associated with this subscription. See the Connect documentation for details.
 * @property null|\StellarPay\Vendors\Stripe\StripeObject $pause_collection If specified, payment collection for this subscription will be paused. Note that the subscription status will be unchanged and will not be updated to <code>paused</code>. Learn more about <a href="/billing/subscriptions/pause-payment">pausing collection</a>.
 * @property null|\StellarPay\Vendors\Stripe\StripeObject $payment_settings Payment settings passed on to invoices created by the subscription.
 * @property null|\StellarPay\Vendors\Stripe\StripeObject $pending_invoice_item_interval Specifies an interval for how often to bill for any pending invoice items. It is analogous to calling <a href="https://stripe.com/docs/api#create_invoice">Create an invoice</a> for the given subscription at the specified interval.
 * @property null|string|\StellarPay\Vendors\Stripe\SetupIntent $pending_setup_intent You can use this <a href="https://stripe.com/docs/api/setup_intents">SetupIntent</a> to collect user authentication when creating a subscription without immediate payment or updating a subscription's payment method, allowing you to optimize for off-session payments. Learn more in the <a href="https://stripe.com/docs/billing/migration/strong-customer-authentication#scenario-2">SCA Migration Guide</a>.
 * @property null|\StellarPay\Vendors\Stripe\StripeObject $pending_update If specified, <a href="https://stripe.com/docs/billing/subscriptions/pending-updates">pending updates</a> that will be applied to the subscription once the <code>latest_invoice</code> has been paid.
 * @property null|string|\StellarPay\Vendors\Stripe\SubscriptionSchedule $schedule The schedule attached to the subscription
 ...
 * @property null|string|\StellarPay\Vendors\Stripe\TestHelpers\TestClock $test_clock ID of the test clock this subscription belongs to.
 * @property null|\StellarPay\Vendors\Stripe\StripeObject $transfer_data The account (if any) the subscription's payments will be attributed to for tax reporting, and where funds from each payment will be transferred to for each of the subscription's invoices.
 * @property null|int $trial_end If the subscription has a trial, the end of that trial.
 * @property null|\StellarPay\Vendors\Stripe\StripeObject $trial_settings Settings related to subscription trials.
...
class Subscription extends ApiResource
{
     ...
     *
     * @return \StellarPay\Vendors\Stripe\Subscription the created resource
     */
     ...
     * @throws \StellarPay\Vendors\Stripe\Exception\ApiErrorException if the request fails
     *
     * @return \StellarPay\Vendors\Stripe\Collection<\Stripe\Subscription> of ApiResources
     */
     ...
     * @throws \StellarPay\Vendors\Stripe\Exception\ApiErrorException if the request fails
     *
     * @return \StellarPay\Vendors\Stripe\Subscription the updated resource
     */
     ...
     * @throws \StellarPay\Vendors\Stripe\Exception\ApiErrorException if the request fails
     *
     * @return \StellarPay\Vendors\Stripe\Subscription the updated subscription
     */
     ...
     * @throws \StellarPay\Vendors\Stripe\Exception\ApiErrorException if the request fails
     *
     * @return \StellarPay\Vendors\Stripe\Subscription the canceled subscription
     */
     ...
     * @throws \StellarPay\Vendors\Stripe\Exception\ApiErrorException if the request fails
     *
     * @return \StellarPay\Vendors\Stripe\Subscription the resumed subscription
     */
     ...
     * @throws \StellarPay\Vendors\Stripe\Exception\ApiErrorException if the request fails
     *
     * @return \StellarPay\Vendors\Stripe\SearchResult<\Stripe\Subscription> the subscription search results
     */
