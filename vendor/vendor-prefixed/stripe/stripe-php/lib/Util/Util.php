...
abstract class Util
{
    ...
    {
        $types = 'v1' === $apiMode ? \StellarPay\Vendors\Stripe\Util\ObjectTypes::mapping
            : \StellarPay\Vendors\Stripe\Util\ObjectTypes::v2Mapping;
        if (self::isList($resp)) {
            $mapped = [];
            foreach ($resp as $i) {
        ...
        if (\is_array($resp)) {
            if (isset($resp['object']) && \is_string($resp['object'])
                ...
                if ('v2' === $apiMode && ('v2.core.event' === $resp['object'])) {
                    $eventTypes = \StellarPay\Vendors\Stripe\Util\EventTypes::thinEventMapping;
                    if (\array_key_exists('type', $resp) && \array_key_exists($resp['type'], $eventTypes)) {
                        $class = $eventTypes[$resp['type']];
                    } else {
                        $class = \StellarPay\Vendors\Stripe\StripeObject::class;
                    }
                }
                ...
                $class = \StellarPay\Vendors\Stripe\V2\Collection::class;
            } else {
                $class = \StellarPay\Vendors\Stripe\StripeObject::class;
            }
        ...
        foreach ($properties as $key => $property) {
            if (\array_key_exists($property->getName(), $json)) {
                if ('related_object' === $property->getName()) {
                    $related_object = new \StellarPay\Vendors\Stripe\RelatedObject();
                    ...
                    $reason = new \StellarPay\Vendors\Stripe\Reason();
    ...
    {
        if ($h instanceof \StellarPay\Vendors\Stripe\ApiResource) {
            return $h->id;
        }
        if (static::isList($h)) {
            $results = [];
