...
trait Request
{
     ...
     *
     * @throws \StellarPay\Vendors\Stripe\Exception\InvalidArgumentException if $params exists and is not an array
     */
    protected static function _validateParams($params = null)
    {
        if ($params && !\is_array($params)) {
            $message = 'You must pass an array as the first argument to Stripe API '
                ...
                . "would be: \"StellarPay\\Vendors\\Stripe\\Charge::create(['amount' => 100, "
                . "'currency' => 'usd', 'source' => 'tok_1234'])\")";

            throw new \StellarPay\Vendors\Stripe\Exception\InvalidArgumentException($message);
        }
    }
    ...
    /**
     * @param string $url URL for the request
     * @param class-string< \Stripe\SearchResult|\StellarPay\Vendors\Stripe\Collection > $resultClass indicating what type of paginated result is returned
     ...
     * @throws \StellarPay\Vendors\Stripe\Exception\ApiErrorException if the request fails
     *
     * @return \StellarPay\Vendors\Stripe\Collection|\StellarPay\Vendors\Stripe\SearchResult
     */
    protected static function _requestPage($url, $resultClass, $params = null, $options = null, $usage = [])
    {
        ...
        if (!($obj instanceof $resultClass)) {
            throw new \StellarPay\Vendors\Stripe\Exception\UnexpectedValueException(
    ...
    {
        $opts = \StellarPay\Vendors\Stripe\Util\RequestOptions::parse($options);
        $baseUrl = isset($opts->apiBase) ? $opts->apiBase : static::baseUrl();
        $requestor = new \StellarPay\Vendors\Stripe\ApiRequestor($opts->apiKey, $baseUrl);
