...
use StellarPay\Vendors\Stripe\Util\Util;
...
class BaseStripeClient implements StripeClientInterface, StripeStreamingClientInterface
{
    ...
    const DEFAULT_CONFIG = [
        ...
        'stripe_account' => null,
        'stripe_context' => null,
        'stripe_version' => \StellarPay\Vendors\Stripe\Util\ApiVersion::CURRENT,
    ...
    /** @var \StellarPay\Vendors\Stripe\Util\RequestOptions */
    private $defaultOpts;
    ...
    public function __construct($config = [])
    {
        if (\is_string($config)) {
            ...
            throw new \StellarPay\Vendors\Stripe\Exception\InvalidArgumentException('$config must be a string or an array');
        }
        ...
        $this->defaultOpts = \StellarPay\Vendors\Stripe\Util\RequestOptions::parse([
    ...
    public function request($method, $path, $params, $opts)
    {
        ...
        $apiMode = \StellarPay\Vendors\Stripe\Util\Util::getApiMode($path);

        $opts = $defaultRequestOpts->merge($opts, true);

        $baseUrl = $opts->apiBase ?: $this->getApiBase();
        $requestor = new \StellarPay\Vendors\Stripe\ApiRequestor($this->apiKeyForRequest($opts), $baseUrl, $this->getAppInfo());
        list($response, $opts->apiKey) = $requestor->request($method, $path, $params, $opts->headers, $apiMode, ['stripe_client']);
        $opts->discardNonPersistentHeaders();
        $obj = \StellarPay\Vendors\Stripe\Util\Util::convertToStripeObject($response->json, $opts, $apiMode);
        if (\is_array($obj)) {
            // Edge case for v2 endpoints that return empty/void response
            // Example: client->v2->billing->meterEventStream->create
            $obj = new \StellarPay\Vendors\Stripe\StripeObject();
        }
     ...
     *
     * @return \StellarPay\Vendors\Stripe\ApiResponse
     */
    public function rawRequest($method, $path, $params = null, $opts = [])
    {
        ...
        $baseUrl = $opts->apiBase ?: $this->getApiBase();
        $requestor = new \StellarPay\Vendors\Stripe\ApiRequestor($this->apiKeyForRequest($opts), $baseUrl);
     ...
     * @param array|\StellarPay\Vendors\Stripe\Util\RequestOptions $opts the special modifiers of the request
     *
     * @return \StellarPay\Vendors\Stripe\Collection|\StellarPay\Vendors\Stripe\V2\Collection of ApiResources
     */
    public function requestCollection($method, $path, $params, $opts)
    {
        ...
        $apiMode = \StellarPay\Vendors\Stripe\Util\Util::getApiMode($path);
        if ('v1' === $apiMode) {
            if (!($obj instanceof \StellarPay\Vendors\Stripe\Collection)) {
                $received_class = \get_class($obj);
                $msg = "Expected to receive `Stripe\\Collection` object from Stripe API. Instead received `{$received_class}`.";

                throw new \StellarPay\Vendors\Stripe\Exception\UnexpectedValueException($msg);
            }
            $obj->setFilters($params);
        } else {
            if (!($obj instanceof \StellarPay\Vendors\Stripe\V2\Collection)) {
                $received_class = \get_class($obj);
     ...
     * @param array|\StellarPay\Vendors\Stripe\Util\RequestOptions $opts the special modifiers of the request
     *
     * @return \StellarPay\Vendors\Stripe\SearchResult of ApiResources
     */
    public function requestSearchResult($method, $path, $params, $opts)
    {
        $obj = $this->request($method, $path, $params, $opts);
        if (!($obj instanceof \StellarPay\Vendors\Stripe\SearchResult)) {
            $received_class = \get_class($obj);
    ...
    /**
     * @param \StellarPay\Vendors\Stripe\Util\RequestOptions $opts
     *
     * @throws \StellarPay\Vendors\Stripe\Exception\AuthenticationException
     *
     * @return string
     */
    private function apiKeyForRequest($opts)
    {
        ...
        if (null === $apiKey) {
            ...
            throw new \StellarPay\Vendors\Stripe\Exception\AuthenticationException($msg);
        }
     ...
     *
     * @throws \StellarPay\Vendors\Stripe\Exception\InvalidArgumentException
     */
    private function validateConfig($config)
    {
        // api_key
        if (null !== $config['api_key'] && !\is_string($config['api_key'])) {
            throw new \StellarPay\Vendors\Stripe\Exception\InvalidArgumentException('api_key must be null or a string');
        }
        ...
        if (null !== $config['client_id'] && !\is_string($config['client_id'])) {
            throw new \StellarPay\Vendors\Stripe\Exception\InvalidArgumentException('client_id must be null or a string');
        }

        // stripe_account
        if (null !== $config['stripe_account'] && !\is_string($config['stripe_account'])) {
            throw new \StellarPay\Vendors\Stripe\Exception\InvalidArgumentException('stripe_account must be null or a string');
        }

        // stripe_context
        if (null !== $config['stripe_context'] && !\is_string($config['stripe_context'])) {
            throw new \StellarPay\Vendors\Stripe\Exception\InvalidArgumentException('stripe_context must be null or a string');
        }

        // stripe_version
        if (null !== $config['stripe_version'] && !\is_string($config['stripe_version'])) {
            throw new \StellarPay\Vendors\Stripe\Exception\InvalidArgumentException('stripe_version must be null or a string');
        }

        // api_base
        if (!\is_string($config['api_base'])) {
            throw new \StellarPay\Vendors\Stripe\Exception\InvalidArgumentException('api_base must be a string');
        }

        // connect_base
        if (!\is_string($config['connect_base'])) {
            throw new \StellarPay\Vendors\Stripe\Exception\InvalidArgumentException('connect_base must be a string');
        }

        // files_base
        if (!\is_string($config['files_base'])) {
            throw new \StellarPay\Vendors\Stripe\Exception\InvalidArgumentException('files_base must be a string');
        }

        // app info
        if (null !== $config['app_info'] && !\is_array($config['app_info'])) {
            throw new \StellarPay\Vendors\Stripe\Exception\InvalidArgumentException('app_info must be an array');
        }
        ...
        if (!empty($extraConfigKeys)) {
            ...
            throw new \StellarPay\Vendors\Stripe\Exception\InvalidArgumentException('Found unknown key(s) in configuration array: ' . $invalidKeys);
        }
    }
     ...
     * @param 'v1'|'v2' $apiMode
     *
     * @return \StellarPay\Vendors\Stripe\StripeObject
     * */
    public function deserialize($json, $apiMode = 'v1')
    {
        return \StellarPay\Vendors\Stripe\Util\Util::convertToStripeObject(\json_decode($json, true), [], $apiMode);
    }
     ...
     *
     * @return \StellarPay\Vendors\Stripe\ThinEvent
     */
    public function parseThinEvent($payload, $sigHeader, $secret, $tolerance = Webhook::DEFAULT_TOLERANCE)
    {
        ...
        try {
            return Util::json_decode_thin_event_object(
                $eventData,
                '\StellarPay\Vendors\Stripe\ThinEvent'
            );
        } catch (\ReflectionException $e) {
            // Fail gracefully
            return new \StellarPay\Vendors\Stripe\ThinEvent();
        }
    }
}
