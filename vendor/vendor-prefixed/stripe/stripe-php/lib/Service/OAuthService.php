...
class OAuthService extends \StellarPay\Vendors\Stripe\Service\AbstractService
{
    /**
     ...
     *
     * @return \StellarPay\Vendors\Stripe\StripeObject the object returned by Stripe's Connect API
     */
    ...
    public function authorizeUrl($params = null, $opts = null)
    {
        ...
        $query = \StellarPay\Vendors\Stripe\Util\Util::encodeParameters($params);
     ...
     * @param null|array $opts
     *
     * @throws \StellarPay\Vendors\Stripe\Exception\OAuth\OAuthErrorException if the request fails
     *
    ...
    private function _getClientId($params = null)
    {
        ...
        if (null === $clientId) {
            $msg = 'No client_id provided. (HINT: set your client_id using '
              . '`new \StellarPay\Vendors\Stripe\StripeClient([clientId => <CLIENT-ID>
     ...
     *
     * @return \StellarPay\Vendors\Stripe\Util\RequestOptions
     */
    private function _parseOpts($opts)
    {
        if (\is_array($opts)) {
            if (\array_key_exists('connect_base', $opts)) {
                ...
                throw new \StellarPay\Vendors\Stripe\Exception\InvalidArgumentException('Use `api_base`, not `connect_base`');
            }
        }

        return \StellarPay\Vendors\Stripe\Util\RequestOptions::parse($opts);
    }
