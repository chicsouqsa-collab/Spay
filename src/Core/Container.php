 ...
 * container
 *
 * This class is responsible for providing container for the application.
 *
...
use StellarPay\Core\Exceptions\Primitives\LogicException;
use StellarPay\Vendors\StellarWP\ContainerContract\ContainerInterface;
use ReflectionClass;
use ReflectionException;
use ReflectionParameter;
use StellarPay\Vendors\Psr\Container\ContainerInterface as PsrContainerInterface;
 ...
 * @since 1.0.0
 */
class Container implements ArrayAccess, ContainerInterface, PsrContainerInterface
{
    /**
    ...
    protected function getClosure(string $abstract, string $concrete): Closure
    {
        return static function ($container, $parameters = []) use ($abstract, $concrete) {
            if ($abstract == $concrete) { // phpcs:ignore SlevomatCodingStandard.Operators.DisallowEqualOperators.DisallowedEqualOperator
    ...
    protected function removeAbstractAlias(string $searched)
    {
        ...
        foreach ($this->abstractAliases as $abstract => $aliases) {
            foreach ($aliases as $index => $alias) {
                if ($alias == $searched) { // phpcs:ignore SlevomatCodingStandard.Operators.DisallowEqualOperators.DisallowedEqualOperator
    ...
    {
        if ($alias === $abstract) {
            throw new LogicException("[{$abstract}] is aliased to itself."); // phpcs:ignore
        }
    ...
    public function get(string $id)
    {
        try {
            ...
            throw new InvalidArgumentException($id, $e->getCode(), $e); // phpcs:ignore
        }
    }
    ...
    public function build($concrete)
    {
        ...
        try {
            ...
            throw new InvalidArgumentException("Target class [$concrete] does not exist.", 0, $e); // phpcs:ignore
        }
    ...
    protected function notInstantiable(string $concrete)
    {
        ...
        throw new BindingResolutionException($message); // phpcs:ignore
    }
    ...
    protected function unresolvablePrimitive(ReflectionParameter $parameter)
    {
        ...
        $message = "Unresolvable dependency resolving [$parameter] in class $name";
