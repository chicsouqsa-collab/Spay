...
/**
 * This class is a contract for facade class.
 *
 * @package StellarPay\Core\Support\Facades
 * @since 1.0.0
 */

declare(strict_types=1);

namespace StellarPay\Core\Support\Facades;
 ...
 * StellarPay Service Container (SC). So by injecting a mock singleton of the decorated class in the SC, it can be mocked.
 *
...
abstract class Facade
{
    ...
    public static function __callStatic(string $name, array $arguments)
    {
        ...
        container()->singletonIf(static::class);
        $staticInstance = container(static::class);

        // Make sure the accessed class is a singleton and get instance
        $accessorClass = $staticInstance->getFacadeAccessor();
        container()->singletonIf($accessorClass);
        $accessorInstance = container($accessorClass);
